  ...
  **/
    ...
    date_default_timezone_set('PRC'); 
      ...
      //控制下单时间
        if($auto){
            //机器人
            $endtime=M('trade')->where('userid=1')->order('addtime desc')->find();
        }else{
            $endtime=M('trade')->where('userid='.userid('','',$auto))->order('addtime desc')->find();
    ...
    if($auto){
            $user_coin = M('UserCoin')->where(array('userid' => 1))->find();
        }else{
            $user_coin = M('UserCoin')->where(array('userid' => userid('','',$auto)))->find();
        }
        ...
        if (C('coin')[$xnb]['fee_bili']) {
            if ($type == 2) {
                ...
                if ($bili_user) {
                    ...
                    if ($bili_keyi) {
                        $bili_zheng = M()->query('select id,price,sum(num-deal)as nums from zhisucom_trade where userid=' . userid('','',$auto) . ' and status=0 and type=2 and market like \'%' . $xnb . '%\' ;');
    ...
    /*if($rmb!="bdb"){      
      ...
      exit;
        ...
        $mo->execute('lock tables zhisucom_status write, zhisucom_trade write ,zhisucom_user_coin write ,zhisucom_finance write,zhisucom_chistory write,zhisucom_config write,zhisucom_user write,zhisucom_coin write');
        $rs = array();
        //$user_coin = $mo->table('zhisucom_user_coin')->where(array('userid' => userid()))->find();  
    if($auto){
            $user_coin = $mo->table('zhisucom_user_coin')->where(array('userid' => 1))->find();
        }else{
            $user_coin = $mo->table('zhisucom_user_coin')->where(array('userid' => userid('','',$auto)))->find();
        }
        if ($type == 1) {
            if ($user_coin[$rmb] < $mum) {
                $this->error(C('coin')[$rmb]['title'] . '余额不足！');
            ...
            $rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => userid('','',$auto)))->setDec($rmb, $mum);  
            //2018-07-18
      ...
      //结束      
            $rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => userid('','',$auto)))->setInc($rmb . 'd', $mum);
            //2018-07-18
      //$rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => userid()))->setInc($xnb . 'd', $mum);
            $rs[] = $finance_nameid = $mo->table('zhisucom_trade')
      ->add(array(
      'userid' => userid('','',$auto), 
      'market' => $market, 
      'price' => $price, 
      ...
      $coinx = strtolower($rmb);
                      ...
                      if($c2c_fee>0)
                      {
                        ...
                        $user1 = $mo->table('zhisucom_user')->where(array('id'=>userid('','',$auto)))->find();
            ...
            $rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => userid('','',$auto)))->setDec($xnb, $num);
            ...
            $rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => userid('','',$auto)))->setInc($xnb . 'd', $num);
            $rs[] = $mo->table('zhisucom_trade')->add(array('userid' => userid('','',$auto), 'market' => $market, 'price' => $price, 'num' => $num, 'mum' => $mum, 'fee' => $fee, 'type' => 2, 'addtime' => time(), 'status' => 0,'trade_pt'=>$trade_pt));
        }
        else {
            $mo->execute('rollback');
        ...
        if (check_arr($rs)) {
            ...
            $mid=M('status')->where($mark)->getField('id');
            if($type == 1){
                parent::addCashhistory(userid('','',$auto),$mid,1,"买入","-".$mum.strtoupper($rmb),$price,$bwallet.strtoupper($rmb),"委托买入".strtoupper($xnb));
            }
            if ($type == 2){
                parent::addCashhistory(userid('','',$auto),$mid,2,"卖出","-".$num.strtoupper($xnb),$price,$bwallet.strtoupper($xnb),"委托卖出".strtoupper($xnb));
            }
    ...
    protected function matchingTrade($market = NULL,$trade_pt = NULL,$auto)#匹配
    {
      ...
      if($auto){
        //机器人只匹配机器人的订单
        $buy = $mo->table('zhisucom_trade')->where(array('market' => $market, 'type' => 1,'userid' => 1, 'status' => 0))->order('price desc,id asc')->find();
        $sell = $mo->table('zhisucom_trade')->where(array('market' => $market, 'type' => 2,'userid' => 1, 'status' => 0))->order('price asc,id asc')->find();
        
      }else{
        $buy = $mo->table('zhisucom_trade')->where(array('market' => $market, 'type' => 1,'userid' => array('gt',1), 'status' => 0))->order('price desc,id asc')->find();
        $sell = $mo->table('zhisucom_trade')->where(array('market' => $market, 'type' => 2,'userid' => array('gt',1), 'status' => 0))->order('price asc,id asc')->find();
        $res['b'] = $buy;
        $res['s'] = $sell;
       // return $res;
      }
