...
//error_log('$trade_fee='.$trade_fee,3,'./bb.txt');
            if ($trade_fee) {
                $fee = round(($num / 100) * $trade_fee, 8);//0.156
...
error_log('$fee='.$fee.'&$mum='.$mum,3,'./bb.txt');
    ...
    public function matchingTrade($market = NULL)
    {
        ...
        for (; true; ) {
			...
			$bvip_fee = $return_vip_fee[1];
			$return_vip_fee = get_vip_fee($sell['userid']);
			$svip_fee = $return_vip_fee[1];
			$fee_buy = C('market')[$market]['fee_buy']*$bvip_fee;
			$fee_sell = C('market')[$market]['fee_sell']*$svip_fee;
            ...
            if ($buy && $sell && (0 <= floatval($buy['price']) - floatval($sell['price']))) {
                ...
                if ($fee_buy) {
                    ...
                    $buy_fee = round(($amount / 100) * $fee_buy, 8);//（120/100*0.2=0.24）
                    //$buy_save = round(($mum / 100) * (100 + $fee_buy), 8);//（120/100*（100+0.2））=120.24
                    //TODO:2018-07-25
                    $buy_save = round($amount-$buy_fee, 8);//（120/100*（100+0.2））=120.24
                }
                else {
                    $buy_fee = 0;
                    $buy_save = $amount;
                ...
                $rs[] = $finance_nameid = $mo->table('zhisucom_trade_log')->add(array('userid' => $buy['userid'], 'peerid' => $sell['userid'], 'market' => $market, 'price' => $price, 'num' => $amount, 'mum' => $mum, 'type' => $type, 'fee_buy' => $buy_fee, 'fee_sell' => $sell_fee, 'addtime' => time(), 'status' => 1,'trade_type'=>0,'trade_id'=>0));
                ...
                $rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => $buy['userid']))->setInc($xnb, $buy_save);
                //$rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => $buy['userid']))->setInc($xnb, $amount);
                
                error_log('buy_save='.$buy_save,3,'./bb123.txt');	
			//账单明细			
				$wallet=M('user_coin')->where("userid=".$buy['userid'])->getField($xnb);
				$mark['curr']=$market;
				$mid=M('status')->where($mark)->getField('id');	
				parent::addCashhistory($buy['userid'],$mid,1,"买入","+".$buy_save.strtoupper($xnb),$price,$wallet.strtoupper($xnb),strtoupper($rmb)."买入".strtoupper($xnb));	
			...
			//结束
                ...
                //减去手续费按最后币种计算的
                $rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => $buy['userid']))->setDec($rmb . 'd', $mum);
				error_log('mum='.$mum,3,'./bb123.txt');	
                //2018-07-18(冻结币种清0)
				...
				error_log('sell_save='.$sell_save,3,'./bb123.txt');	
                ...
                $buy_list = $mo->table('zhisucom_trade')->where(array('id' => $buy['id'], 'status' => 0))->find();
					...
					error_log('save_sell_xnb='.$save_sell_xnb.'\n\r',3,'./bb123.txt');	
				...
				//赠送手续费等量BDB
					...
					if($buy_fee>0){							
						$coin['name']=$xnb."_bdb";
						$buyprice=$mo->table('zhisucom_market')->where($coin)->getField("new_price");		
						//获取币种对BDB的最新价格
					...
					//	error_log('name='.$market.'buyprice='.$buyprice.'$buy_fee='.$buy_fee,3,'./bb12.txt');	
						unset($buyfee);
						//结束
					}		
					//卖出
