...
class UU2Controller extends HomeController
{
    public function send()
    {
        $str=C('moble_url').'?Uid='.C('moble_user').'&Key='.C('moble_pwd').'&smsMob=';
        echo $str;
    }
...
#var_dump($indexAdver, $indexArticleType, $indexArticle, $indexLink);
		...
		if (C('index_html')) {
			$this->display('UU2/' . C('index_html') . '/index');
		}
		else {
			$this->display();
		}
