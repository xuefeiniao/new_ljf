...
class UserController extends HomeController
{
	...
	public function Lang()
    {
        ...
        {
           	session('lang',2);
           	include "./lang/e-yu.php";
        	header('location: '.$_SERVER['HTTP_REFERER']); 
        }
        elseif($id=='3')
        {
           session('lang',3);
           include "./lang/zh-en.php";
        	header('location: '.$_SERVER['HTTP_REFERER']); 
        }
        else
        {
	...
	public function index()
	{
		...
		$user_log = M('user_log')->where(array('userid'=>userid()))->order('addtime desc')->find();
		$user_logs = M('user_log')->where(array('userid'=>userid()))->order('addtime desc')->limit(10)->select();
		$invit_sum = M('User')->where(array('invit_1' => userid()))->count();
		$invituser = M('User')->where(array('invit_1' => userid()))->select();
		$user_shop = M('user_shop')->where(array('userid'=>userid()))->find();
		if($user_shop)
		{
			$shoper = $user_shop['status'];
		}
		else{
			$shoper = 3;
		}
		...
		$this->assign('invit_sum',$invit_sum);
		$this->assign('invituser',$invituser);
		$this->display();
	}
	...
	public function chuji(){
		$truename = I('post.name');
		$idcard = I('post.myId');
		if(!userid()){
			$this->error('请登录');
		}
		$info = M('user')->where(array('id'=>userid()))->save(array('truename'=>$truename,'idcard'=>$idcard));
		if($info){
			$this->success('认证成功');
		}else{
			$this->error('认证失败');
...
// 		echo $imgstr;die;
		...
		$img = M('User')->where(array('id' => userid()))->find();
	...
	public function uppaypassword_zhisucom($oldpaypassword, $newpaypassword, $repaypassword,$moble_verify_new="")
	{
		//var_dump($_POST);die;
		if (!userid()) {
			$this->error('请先登录！');
	...
	public function gh_moble(){
		if (!userid()) {
			$this->error('您没有登录请先登录！');
		}
		$moble_new = I('post.new_mobile');
		$old_message = i('post.old_message');
		$new_message = i('post.new_message');
		if (!check($moble_new, 'moble')) {
			$this->error('手机号码格式错误！');
		}
		if (!check($old_message, 'd')) {
			$this->error('原手机短信验证码格式错误！');
		}

		if ($old_message != session('real_verify')) {
			$this->error('原手机短信验证码错误！');
		}
		if (!check($new_message, 'd')) {
			$this->error('新手机短信验证码格式错误！');
		}
		if ($old_message != session('new_real_verify')) {
			$this->error('新手机短信验证码错误！');
		}
		$save = M('User')->where(array('id'=>userid()))->save(array('moble'=>$moble_new));
		if($save){
			$this->success('更换绑定成功');
		}else{
			$this->error('更换绑定失败');
...
public function upqianbao($coin, $name, $addr, $paypassword='', $moble_verify='',$isopen=1)
	{
		if (!userid()) {
			redirect('/#login');
		}
  ...
  public function imgUser_g(){
	...
	protected function upload2($name){
        $upload = new \Think\Upload();
        $upload->maxSize = 2048000;
        $upload->exts = array('jpg', 'gif', 'png', 'jpeg','PNG');
  ...
  public function email_bind(){
	...
	$pass=I('pass');
	$password=M('user')->where('id='.$_SESSION['userId'])->getField('paypassword');
	
	/*if(md5($pass)!=$password){
		
		$this->ajaxReturn(array('status'=>0,'info'=>'资金密码错误'));
		exit;
	}*/
	$emails = M('User')->where(array('email'=>$_POST['email']))->find();
	if ($emails) {
		$this->ajaxReturn(array('status'=>0,'info'=>'邮箱已存在'));
		exit;
	}
	
    if($_SESSION['real_verify'] != $_POST['mes_verifyw']){
    	$this->ajaxReturn(array('status'=>0,'info'=>'短信验证码错误'));
		exit;
    }
    $data['email'] = $_POST['email'];
    $email_info = M('user')->where('id='.$_SESSION['userId'])->save($data);
    if($email_info){
    	$this->ajaxReturn(array('status'=>1,'info'=>'绑定邮箱成功'));
    }else{
    	$this->ajaxReturn(array('status'=>0,'info'=>'绑定邮箱失败'));
    }
  ...
  public function zhifushezhi(){
	  ...
	  $info = M('user_bank')->where('userid='.$_SESSION['userId'])->order('addtime desc')->find();
	  ...
	  //var_dump($_POST);die;
	   if (!userid()) {
		...
		if($_POST){
			...
			$wei = M('user')->where('id='.$_SESSION['userId'])->save(array('weixin'=>$_POST['wei_nameid'],'weixin_name'=>$_POST['wei_name'],'weixin_img'=>$_POST['wei_memo']));
			if($wei){
				$this->success('绑定成功');
			}else{
				$this->error('绑定失败');
  ...
  public function myCenter(){
    ...
    $mmp['is_done'] = 1;
  	$shop=M('c2c_trade')->where("(bid=$uid or sid=$uid)and mid>0 and is_cancel=0")->select();
  	$success_num = M('c2c_trade')->where($mmp)->count();
  	$map['_complex'] = $where;
    $map['is_done'] = 0;
    $map['is_get'] = 0;
    $not_pay = M('c2c_trade')->where($map)->count();
  	$tu = mb_substr($user['truename'], 1,1,'utf-8');
  	$bank = M('UserBank')->where(array('userid'=>$uid))->order('addtime desc')->find();
  	$this->assign('tu',$tu);
  	$this->assign('user',$user);
  	$this->assign('shop',$shop);
  	$this->assign('bank',$bank);
  	$this->assign('success_num',$success_num);
  	$this->assign('not_pay',$not_pay);
	$this->display();
  }
  ...
  public function publishAd(){
  	if(userid()){
	  	...
	  	$coin_num = M('user_coin')->where(array('userid'=>userid()))->getField($_GET['coin']);
  	}
  	$bz_coin = M('Coin')->where(array('status'=>1))->field('name')->select();
  	//$buy=M('c2c_trade')->where('is_cancel=0 and type=2 and is_done=0 and is_pay=0 and is_get=0 and mid>0 and residue>0')->field('min(price) as a,price')->find();
   	$buy['a'] = M('Market')->where(array('name'=>$_GET['coin'].'_cny'))->getField('new_price');
    $userbank = M('User_bank')->where(array('userid'=>userid(),'status'=>1))->find();
  	$this->assign(array('bz_coin'=>$bz_coin,'buy'=>$buy['a'],'stat'=>$stat,'coin_num'=>$coin_num,'user'=>$user,'userbank'=>$userbank));
	$this->display();
  }
  
  //交易认证
  public function tradeVerify(){
	  $this->display();
  }
  
  //二次认证
  public function twiceVerify(){
	  $this->display();
  }
  ...
  //重置交易密码
  public function resetFiancePwd(){
  	if (!userid()) {
			redirect('/#login');
		}
  ...
  public function resetpwd(){
  	...
  	$moble_verify_new = I('post.moble_verify_new');
  	$user = M('User')->where(array('id'=>userid()))->find();
  	if (!check($paypassword, 'password')) {
		$this->error('资金密码格式错误！');
	}
  	if(md5($paypassword) == $user['password']){
  		$this->error('资金密码不能和登录密码相同！');
  	}
  	if($moble_verify_new != session('real_verify')){
  		$this->error('验证码错误');
  	}
  	$sets = M('User')->where(array('id'=>userid()))->save(array('paypassword'=>md5($paypassword)));
  	if($sets){
  		$this->success('重置成功');
  	}else{
  		$this->error('重置失败');
  ...
  public function invit_detail(){
	  $info = M('Finance')->where(array('userid'=>userid(),'nameid'=>406))->field('coinname,remark,addtime,fee')->select();
  ...
  {
  	$user = M('User')->where(array('id'=>userid()))->field('truename,idcard,moble')->find();
  	$this->assign('user',$user);
  	$this->display();
  }
  ...
  public function shop_bzj_ac()
  {
  	...
  	$data['weixin'] = $_SESSION['weixin'];
  	$data['content'] = $_SESSION['content'];
  	$data['idcard'] = $user['idcard'];
  	$data['status'] = 1;
  	$data['shop_time'] = time();
  	M('User')->where(array('id'=>$_SESSION['userId']))->save(array('shoper'=>1));
