...
class TradeController extends HomeController
{
    ...
    public function upTrade($paypassword = NULL, $market = NULL, $price=0, $num, $type,$tradetype)
    {
        ...
        if ($type == 1) {
            ...
            $rs[] = $finance_nameid = $mo->table('zhisucom_trade')->add(array('userid' => userid(), 'market' => $market, 'price' => $price, 'num' => $num, 'mum' => $mum, 'fee' => $fee1, 'type' => 1, 'addtime' => time(), 'status' => 0));
    ...
    protected function matchingTrade($market = NULL)
    {
        if (!$market) {
            return false;
        }
