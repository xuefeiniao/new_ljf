...
/*    foreach ($CoinList as $k => $v) {
  ...
  //  var_dump($coinList);die;
    ...
    public function mydz(){
      $coin = $_GET['coin'];
      $qianbao = M('User_qianbao')->where(array('userid'=>userid(),'coinname'=>$coin,'status'=>1))->select();
...
//var_dump($qianbao_addr);die;
  ...
  public function turnout(){
    ...
    if($_POST['turnout'] == 0 && $_POST['turnin'] == 1){#从币币账户转向杠杆交易用户
    ...
    elseif($_POST['turnout'] == 1 && $_POST['turnin'] == 0){#从杠杆交易账户转向币币交易账户
  ...
  public function qianbao111($coin = NULL)
  {
    if (!userid()) {
      redirect('/#login');
    }
  ...
  public function myzc($coin = NULL)
  {
    ...
    $yy = M('user_qianbao')->where(array('userid' => userid(), 'status' => 1, 'coinname' => $coin))->order('addtime desc')->find();
    $ky_sum = M('User_coin')->where(array('userid'=>userid()))->getField($coin);
    $where['userid'] = userid();
    $where['coinname'] = $coin;
    $Moble = M('Myzc');
    $count = $Moble->where($where)->count();
...
/*$coin = I('post.coin');
  ...
  // echo $coin;die;
    ...
    {
        $from_addr = M('User_coin')->where(array('userid'=>userid()))->find();
      ...
      $_val = json_encode($val);
      $sign = sha1(md5($_val));
      $postFields = ['coin_en'=>$coin,'addr'=>$addr,'num'=>$mum,'sign'=>$sign,'from_addr'=>$from_addr['amfcb']];
     //  var_dump($postFields);
      $ch = curl_init ();
      curl_setopt( $ch, CURLOPT_URL, $url );
      curl_setopt( $ch, CURLOPT_RETURNTRANSFER, 1 );
    ...
    if ($Coin['type'] == 'rgb') {
      ...
      //账单明细      
        ...
        parent::addCashhistory(userid(),$mid,7,"提币",-$num,0,$wallet.strtoupper($coin),"提币".strtoupper($coin));        
      //结束
      ...
      if ($fee) {
        if ($mo->table('zhisucom_user_coin')->where(array($qbdz => $Coin['zc_user']))->find()) {
          ...
          debug(array('msg' => '提币收取手续费' . $fee), 'fee');
        }
        else {
    ...
    if ($Coin['type'] == 'qbb') {
      $mo = M();
      //dump($mo->table('zhisucom_user_coin')->where(array($qbdz => $addr))->find());die;
   ...
   die;*/
        ...
        $auto_status=0;
        $insertdata=array('userid' => userid(), 'username' => $addr, 'coinname' => $coin, 'num' => $num, 'fee' => $fee, 'mum' => $mum, 'addtime' => time(), 'status' => $auto_status);
  ...
  //充值记录
  public function rechargeRecord($coin){
      $where['userid'] = userid();
    $where['coinname'] = $coin;
    $Moble = M('Myzr');
    $count = $Moble->where($where)->count();
  ...
  //地址切换
  public function dzac(){
    $id = I('post.id');
    $info = M('user_qianbao')->where(array('id'=>$id))->save(array('addtime'=>time()));
    if($info){
      $this->success('切换成功');
    }else{
      $this->error('切换失败');
  ...
  $id = I('post.id'); 
  $info = M('user_qianbao')->where(array('id'=>$id))->delete();
    if($info){
      $this->success('解绑成功');
    }else{
      $this->error('解绑失败');
