...
{
	 //杠杆交易
    public function borrowMoney($market = NULL){
        $showPW = 1;
       ...
       // $bow_jtc = M('User_coin')->where(array('userid'=>userid()))->getField('jtcbow');
		$bow = M('lever_coin')->where(array('userid'=>userid(),'name_en'=>$data['s1']))->find();
        $this->assign('bow', $bow);
		...
		if(($fxl*100) <= $config['lever_fx'] + 5 && $fxl>0)
		{
			...
			if($levers)
			{
				foreach($levers as $k => $v)
				{
					...
					if(!$repay)
					{
						$this->upTrade(NULL,$where,$new_price,$lever_coin['borrow'],2,2,2);
					}
				}
	...
	**/
    public function upTrade($paypassword = NULL, $market = NULL, $price=0, $num=NULL, $type,$tradetype,$trade_pt,$auto=0)
    {
		...
		if($auto){
            ...
            $user_coin = $mo->table('zhisucom_lever_coin')->where(array('userid' => userid('','',$auto),'name_en'=>$xnb))->find();
        }
	...
	//利息每天自增长
	public function lixi_add()
	{
		$lever = M('Lever')->where(array('status'=>1))->select();
		foreach($lever as $k=>$v)
		{
			$lixi = $v['borrow'] * ($v['interest_fee'] / 100);
			//echo '<pre/>';
			//echo $lixi;
			M('Lever')->where(array('b_order'=>$v['b_order'],'status'=>1))->setInc('interest',$lixi);
		}
	}
	
	#历史委托
    public function getTradels($market=null,$trade_pt=1)
    {
        $uid=userid();
        $market=I('get.market');
