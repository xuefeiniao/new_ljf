...
class FinanceController extends AdminController
{
  ...
  public function index_out(){
  	$column = ["ID","用户名","操作币种","操作数量","操作类型","操作之前","操作之后","操作说明","操作时间"];
    $name_list = array('mycz' => '人民币充值', 'mytx' => '人民币提现', 'trade' => '委托交易', 'tradelog' => '成功交易', 'issue' => '用户认购');
    $data = M('Finance')->order('id desc')->select();
    foreach ($data as $k => $v) {
      		$list[$k]['id'] = $v['id'];
			$list[$k]['username'] = M('User')->where(array('id' => $v['userid']))->getField('username');
    ...
    excelOutput($list,$column,"财务明细[".date("Y-m-d H:i:s")."]","财务明细-".date("Y-m-d H:i:s"));
  }
  ...
  public function myzr_out()
  {
  	$data = M('Myzr')->order('id desc')->select();
    $column = ["ID","用户名","币种","转入时间","转出地址","转入数量","实际到账","转入赠送","状态"];
    foreach ($data as $k => $v) {
      		$list[$k]['id'] = $v['id'];
			$list[$k]['usernamea'] = M('User')->where(array('id' => $v['userid']))->getField('username');
            ...
            $list[$k]['mum'] = $v['mum'];
            $list[$k]['fee'] = $v['fee'];
            $list[$k]['status'] =$v['status'] == 1?'转入成功':'等待处理';
		}
    excelOutput($list,$column,"充币明细[".date("Y-m-d H:i:s")."]","充币明细-".date("Y-m-d_H:i:s"));
  }
   ...
   public function myzc_out()
  {
  	$data = M('Myzc')->order('id desc')->select();
    $column = ["ID","用户名","币种","申请时间","转出地址","转出数量","提现手续费","状态"];
    foreach ($data as $k => $v) {
      		$list[$k]['id'] = $v['id'];
			$list[$k]['usernamea'] = M('User')->where(array('id' => $v['userid']))->getField('username');
      		$list[$k]['coinname'] = strtoupper($v['coinname']);
    ...
    excelOutput($list,$column,"提币明细[".date("Y-m-d H:i:s")."]","提币明细-".date("Y-m-d_H:i:s"));
  }
