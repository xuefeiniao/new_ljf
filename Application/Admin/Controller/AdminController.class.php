...
class AdminController extends \Think\Controller
{
    public function __construct()
    {
        ...
        if ($access === false) {
            ...
            if ($dynamic === null) {
                ...
                if($rule != 'admin/index/index')//"admin/index/index"，默认是系统-系统概览菜单，将index放开了，可改为欢迎界面
                {
		...
		# 817改写菜单
		$cate = M('menu')->where('hide=0 and pid=0')->field('id,title,url')->order('sort')->select();
		foreach($cate as $k=>$v)
		{
			$cate[$k]['subcate'] = getCate($v['id']);
		}
	...
	private function GetTree($arr,$pid){
    global $tree;
    ...
    ******************************************************/
	...
	public function addCashhistory($id,$coin,$type,$action,$num,$price,$wallet,$detail="")
	{
        $chistory=M('chistory');
        //找最近的当前的货币总额
     //   $fck_rs=$this->where("id=".$ID)->field('agent_cash,agent_use')->find();
       // if($type==1 || $type==3){
