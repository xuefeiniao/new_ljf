...
for($i=0;$i<count($array_file);$i++){
  ...
  return $date;
    ...
    //机器人
    public function automarket($market = NULL,$new_price='',$total_num='')
    {
        if (!$market) {
            $market = C('market_mr');
        }
        ...
        if (!$market) {
            echo '请去[设置--其他设置]里面设置默认市场';
            exit();
        }
        $hb=A('Home/Huobi');
        $mket=M('market')->where(['status'=>1,'trade'=>1])->field('name,new_price')->select();
        foreach (C('MARKET') as $k => $v){
        	$list[$k]['name']=$v['name'];
        	$list[$k]['title']=$v['title'];
            $coin = str_replace('_','',$v['name']);
            $hb_price=$hb->hangqing($coin);
            if ($hb_price['status']!=='error'){
            	$price_e=$hb_price['tick']['close'];
                if (strpos($price_e,'E') > 1){
                   // $list[$k]['new_price']=sctonum($price_e,8);
                  	$list[$k]['new_price']=$price_e;
                }else{
                    $list[$k]['new_price']=$price_e;
				}
            }else{
                $list[$k]['new_price']=$v['new_price'];
			}
        }
        $this->assign('list', $list);
    ...
    //添加任务
    public function automarket_add($market='',$new_price='',$total_num=1,$iszhang=1,$min_price=1,$max_price=10,$min_num=0.001,$max_num=10){
        ...
        $add['min_num'] = $min_num;
        $add['max_num'] = $max_num;
        $add['yu_num'] = $total_num;
        $res = M('automan')->add($add);
        
        if ($res) {
            echo json_encode(array('status' => 1, 'info' => '机器人任务添加成功'));
			exit();
		}
		else {
		    echo json_encode(array('status' => 0, 'info' => '机器人任务添加失败'));
			exit();
		}
    }
    ...
    //
    //任务列表
    public function automan_list(){
		$count = M('automan')->where($where)->count();
		$Page = new \Think\Page($count, 10);
		$show = $Page->show();
		$list = M('automan')->where($where)->order('id desc')->limit($Page->firstRow . ',' . $Page->listRows)->select();
    ...
    //停止任务
    public function end_automan($id = NULL,$type){
        if (empty($id)) {
			$this->error('请选择任务列表！');
		}
		...
		switch (strtolower($type)) {
		    case 'end':
		        $rel = $moble->where($where)->setField('isend',1);
        		if ($rel!==FALSE) {
        			$this->success('操作成功！');
        		}
        		else {
			    ...
			    break;
			case 'start':
		        $rel = $moble->where($where)->setField('isend',2);
        		if ($rel!==FALSE) {
        			$this->success('操作成功！');
        		}
        		else {
	...
	//删除任务
	public function del_automan()
	{
		$id = I('post.id');
		if($id > 0)
		{
			$where['id'] = $id;
			$rel = M('Automan')->where($where)->delete();
			if($rel)
			{
				$this->success('删除成功');
			}
	...
	//杠杆借款 
	public function lever_borrow($field = NULL, $name = NULL, $market = NULL, $status = NULL)
	{
		$where = array();
...
//$res = M('Lever')->where($where)->select();
		$count = M('Lever')->where($where)->count();
		$Page = new \Think\Page($count, 15);
		$show = $Page->show();
		$list = M('Lever')->where($where)->order('id desc')->limit($Page->firstRow . ',' . $Page->listRows)->select();
//dump($list);die;
	...
	//杠杆还款记录
	public function lever_repayment($field = NULL, $name = NULL, $market = NULL, $status = NULL)
	{
		$where = array();
