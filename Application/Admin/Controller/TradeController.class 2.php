...
class TradeController extends AdminController
{
	...
	public function market($field = NULL, $name = NULL)
	{
		...
		$this->assign('list', $list);dump($list);die;
		$this->assign('page', $show);
		$this->display();
	}
	...
	public function c2c($field = NULL, $name = NULL, $stat=NULL, $status = NULL){
        $where = array();
        if ($field && $name) {
            if ($field == 'username') {
                $where['userid'] = M('User')->where(array('username' => $name))->getField('id');
        ...
        foreach ($list as $k => $v) {
            $user= M('User')->where(array('id' => $list[$k]['userid']))->find();
            $list[$k]['username']=$user['username'];
            $list[$k]['mobile']=$user['moble'];
        }
