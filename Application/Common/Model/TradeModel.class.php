...
class TradeModel extends \Think\Model
{
	...
	public function chexiao($id = NULL)
	{
		...
		$mo = M();
		$mo->execute('set autocommit=0');
		$mo->execute('lock tables zhisucom_user_coin write  , zhisucom_trade write ,zhisucom_finance write ,zhisucom_chistory write , zhisucom_status write');
		$rs = array();
		...
		if ($trade['type'] == 1) {
			...
			if (!$you_buy) {
			    ...
			    if (0 < $you_user_buy[$rmb . 'd']) {
			        $mo->table('zhisucom_user_coin')->where(array('userid' => $trade['userid']))->setField($rmb . 'd', 0);
			    }
			}
		}
		...
		if (check_arr($rs)) {
			...
			//TODO 2018-07-28
			//撤销的订单进入账单明细表
			$coin = M('status')->where(array('curr'=>$trade['market']))->getField('id');
			$chistory=M('chistory');
			$data=array();
			$nowtime=time();
			$data['uid']=$trade['userid'];
			$data['coin']= $coin;
			$data['time']=$nowtime;
			...
			if($trade['type']==1){
			    //买
			    $wallet=M('user_coin')->where("userid=".$trade['userid'])->getField($rmb);
			    $data['wallet']=$wallet?$wallet.strtoupper($rmb):'0'.strtoupper($rmb);
			    
			    $yu_num = $trade['mum']-$trade['deal'];
			    $num = '+'.$yu_num.strtoupper($rmb);
			}else{
			    //卖
			    $wallet=M('user_coin')->where("userid=".$trade['userid'])->getField($xnb);
			    $data['wallet']=$wallet?$wallet.strtoupper($xnb):'0'.strtoupper($xnb);
			    
			    $yu_num = $trade['num']-$trade['deal'];
			    $num = '+'.$yu_num.strtoupper($xnb);
			}
			...
			$data['action_type']='委托撤单';
			$data['mod']=$mod;
				
			if($trade['type']==1){
			    $detail = '撤销委托买入'.strtoupper($xnb);
			}else{
			    $detail = '撤销委托卖出'.strtoupper($xnb);
			}
			$data['detail']=$detail;
			$chistory->add($data);
			//结束
