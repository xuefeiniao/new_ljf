...
{
    $cate = M('menu')->where('hide=0 and pid='.$pid)->field('id,title,url')->order('sort')->select();
    return $cate;
}


//根据用户VIP级别获取手续费率
function get_vip_fee($userid){
    //计算月交易量(当天起往上30天)
    
    /*$endtime = strtotime(date("Y-m-d 23:59:59",strtotime("-1 day")));
    $starttime = $endtime-30*24*3600;
    $all_trade_num = M()->query('select sum(num) as nums from zhisucom_trade_log where userid=' . $userid . ' OR peerid='.$userid.' and status=1 and (addtime>='.$starttime.' or addtime<'.$endtime.') ;');

    
    
    
    $nums = $all_trade_num[0]['nums'];//一个月总交易量*/
    //error_log('$nums='.$nums,3,'./bb.txt');
    
    $total_bdb_nums = sum_market_num($userid);
    if($total_bdb_nums){
        $new_price = M('market')->where(array('name'=>'btc_bdb'))->getField('new_price');
        $nums = round($total_bdb_nums/$new_price,5);
        $vip_first = 0;//按月交易量计算VIP级别
        for($i=7;$i>=1;$i--){
            $vip_name = 'vip'.$i.'_limit';
            $vip_limit = M('config')->where("id=1")->getField($vip_name);
            if($nums >= $vip_limit){
                $vip_first = $i;
                break;
            }
    ...
    $vip_first = $vip_first?$vip_first:0;
    //
error_log('$userid='.$userid.'&$total_bdb_nums='.$total_bdb_nums.'&$nums='.$nums.'&$vip_first='.$vip_first."\r\n",3,'./bb.txt');
    //资产全折合成btc
    $zong_zc = sumCoin($userid);
//error_log('$zong_zc='.$zong_zc,3,'./bb.txt');
    if($zong_zc){
        //$new_price = M('market')->where(array('name'=>'btc_usdt'))->getField('new_price');
        $new_price = M('market')->where(array('name'=>'btc_bdb'))->getField('new_price');
//
        $price = round($zong_zc/$new_price,2);
//error_log('$userid='.$userid.'$new_price='.$new_price.'&price='.$price,3,'./bb.txt');
        $vip_second = 0;//按余额等额兑换BTC计算VIP级别
        for($i=7;$i>=1;$i--){
            $vip_name = 'vip'.$i.'_limit';
            $vip_limit = M('config')->where("id=1")->getField($vip_name);
            if($price >= $vip_limit){
    ...
    //
//error_log('$vip_second='.$vip_second,3,'./bb.txt');    
    $vip = $vip_first>=$vip_second?$vip_first:$vip_second;//取最大级别
//error_log('$userid='.$userid.'$vip='.$vip."\r\n",3,'./bb.txt');
    //根据最终VIP获取对应VIP级别的佣金折扣
    if($vip){
        $fee_name = 'vip'.$vip.'_fee';
        $vip_fee = M('config')->where("id=1")->getField($fee_name);
        $vip_fee = $vip_fee?$vip_fee:0;
    }else{
        $vip_fee = 0;
    }
//error_log('$userid='.$userid.'$vip_fee='.$vip_fee."\r\n",3,'./bb.txt');
    $vip_fee = $vip_fee==0?1:round($vip_fee/100,5);//0.65
    $return_arr = [];
    $return_arr[0] = $vip;
    $return_arr[1] = $vip_fee;
    return $return_arr;
}


//计算30天总交易量等价bdb
function sum_market_num($userid){
    //计算月交易量(当天起往上30天)
    ...
    $endtime = strtotime(date("Y-m-d 23:59:59",strtotime("-1 day")));
    $starttime = $endtime-30*24*3600;
    $all_trade_list = M()->query('select * from zhisucom_trade_log where userid=' . $userid . ' OR peerid='.$userid.' and status=1 and (addtime>='.$starttime.' or addtime<'.$endtime.') ;');
    //echo '<pre>';
    //print_r($all_trade_list);
    
    if($all_trade_list){
        $new_arr_big = [];
        $new_arr = [];
        foreach ($all_trade_list as $v){
            $xnb = explode('_', $v['market']);
            $new_arr['num'] = $v['num'];
            $new_arr['name'] = $xnb[0];
            array_push($new_arr_big, $new_arr);
        }
        
        //echo '<pre>';
        //print_r($new_arr_big);
        
        foreach ($new_arr_big as $k=>$v){
            if($v['name']=='bdb'){
                $zj += $v['num'];
            }else{
...
function userid123($username = NULL, $type = 'username')
{
	if ($username && $type) {
		...
		//app端使用到
		if(!$userid){
		    $userid = M('user')->where(array('login_token'=>trim($_REQUEST['ltoken'])))->getField('id');
		}
	}
...
function CoinClient($username, $password, $ip, $port, $timeout = 30, $headers = array(), $suppress_errors = false)
{
	return new \Common\Ext\CoinClient($username, $password, $ip, $port, $timeout, $headers, $suppress_errors);
}

function EthCommon($ip, $port='80', $version='2.0')
{
	return new \Common\Ext\EthCommon($ip, $port, $version);
}
...
function check_google($code,$id){
	
	$key=M('user')->where('id='.$id)->getField('secret');	
	$ga=new \Home\Controller\GoogleAuthenticatorController();	
	$status=$ga->verifyCode($code,$key);	
	return $status;	
}
...
function send_moble($moble, $content)
{
	...
	*/
	...
	$name = 'JTC';//网站的名称
	$content = $content;
	//$url = 'http://utf8.api.smschinese.cn/?Uid=jiutianwenhua&Key=f85a0d55ffe60c024a0e016cd35cde3c&smsMob=' . $moble . '&smsText=' . $content;
	$url = C('moble_url').'/?Uid='.C('moble_user').'&Key='.C('moble_pwd').'&smsMob=' . $moble . '&smsText=' . $content;
...
function check($data, $rule = NULL, $ext = NULL)
{
	...
	$validate['username'] = '/^[a-zA-Z]{2,15}|[0-9a-zA-Z_]{2,15}$/';
	$validate['email'] = '/^\\w+([-+.]\\w+)*@\\w+([-.]\\w+)*\\.\\w+([-.]\\w+)*$/';
	$validate['moble'] = '/^(((1[0-9][0-9]{1})|159|153)+\\d{8})$/';
	$validate['old_password'] = '/^[a-zA-Z0-9_\\@\\#\\$\\%\\^\\&\\*\\(\\)\\!\\,\\.\\?\\-\\+\\|\\=]{6,16}$/';
	$validate['password'] = '/^(?![0-9]+$)(?![a-zA-Z]+$)[0-9a-zA-Z]{6,16}+$/';//至少6位字符，非纯数字
 ...
 */
 ...
 //生产单号
	 ...
	 * @param string $msg 			短信内容,必须和253平台备案模板一致
	 * @param string $needstatus 	是否需要状态报告
	 */
	function send_mobile($mobile, $msgs, $needstatus = 'true')
	{
		
		//$content = "您的验证码是".$code."，请勿泄漏，谨防被骗。";
		$sign = "【JEFFEX】";//必须和253平台备案的签名一致
		$msg = $sign.$msgs;
		//创蓝接口参数
		$postFields = array (
			'account'  =>  "N663466_N6750627",// 创蓝API账号
			'password' => "3B2maErygi5b36",// 创蓝API密码
			'msg' => urlencode($msg),
			'phone' => $mobile,
			'report' => $needstatus,
       		);
