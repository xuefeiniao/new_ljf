...
error_log('$fee_buy='.$fee_buy.'&$fee_sell='.$fee_sell."\r\n",3,'./bb.txt');
        ...
        for (; true; ) {
            ...
            if ($buy && $sell && (0 <= floatval($buy['price']) - floatval($sell['price']))) {
                ...
                //2018-07-18
                /*if ($buy_save <= round($user_buy[$xnb . 'd'], 8)) {
                    $save_buy_rmb = $buy_save;
                }
                else if ($buy_save <= round($user_buy[$xnb . 'd'], 8) + 1) {
                    $save_buy_rmb = $user_buy[$xnb . 'd'];
                    //else if ($buy_save > round($user_buy[$rmb . 'd'], 8)) {
                    //$save_buy_rmb = $buy_save;
                    $log = '错误8交易市场' . $market . '出错：买入订单:' . $buy['id'] . '卖出订单：' . $sell['id'] . '交易方式：' . $type . '成交数量' . $amount . '成交价格' . $price . '成交总额' . $mum . "\n";
                    $log .= 'ERR: 买家更新冻结人民币出现误差,应该更新' . $buy_save . '账号余额' . $user_buy[$xnb . 'd'] . '实际更新' . $save_buy_rmb;
                    mlog($log);
                }
                else {
                    $log = '错误9交易市场' . $market . '出错：买入订单:' . $buy['id'] . '卖出订单：' . $sell['id'] . '交易方式：' . $type . '成交数量' . $amount . '成交价格' . $price . '成交总额' . $mum . "\n";
                ...
                //2018-07-18(购买币种数量增加)
                //$rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => $buy['userid']))->setInc($rmb, $amount);
			...
			//结束
                ...
                if (!$you_buy) {
                    ...
                    //2018-07-18
                    /*if (0 < $you_user_buy[$xnb . 'd']) {
                        $rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => $buy['userid']))->setField($xnb . 'd', 0);
                        $rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => $buy['userid']))->setInc($xnb, $you_user_buy[$xnb . 'd']);
                    }*/
                }
