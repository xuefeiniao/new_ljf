...
class LoginController extends HomeController
{
	...
	public function sms_code()
	{
		...
		if($res)
		{
			$codes = M('Code_log')->add(array('username'=>$username,'code'=>$code,'addtime'=>time(),'type'=>1));
	...
	public function upregister_C()
	{
	    ...
	    // dump($moble_verify);dump(session('real_verify'));die;
		if(M_ONLY==0){
		...
		if ($moble_verify != $_SESSION['real_verify']) {
			$this->error('短信验证码错误！');
		}*/
		...
		if (M('User')->where(array('moble' => $moble))->field('id')->find()) {
		    // $this->error('手机号已存在');
		    $this->ajaxReturn(array('code'=>'1027'));
		}
		...
		$rs = array();
		$login_token = md5($username.$password);//app端平台用到
		$rs[] = $mo->table('zhisucom_user')->add(array('username' => $username,'moble' => $moble,'mobletime' => time(), 'password' => md5($password), 'invit' => $tradeno, 'tpwdsetting' => 3, 'invit_1' => $invit_1, 'invit_2' => $invit_2, 'invit_3' => $invit_3, 'addip' => get_client_ip(), 'addr' => get_city_ip(), 'addtime' => time(), 'status' => 1,'login_token'=>$login_token));
		$rs[] = $mo->table('zhisucom_user_coin')->add(array('userid' => $rs[0]));
