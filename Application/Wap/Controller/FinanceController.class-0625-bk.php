...
/* 		foreach ($CoinList as $k => $v) {
	...
	public function mytxUp($price, $num)
    {
        if (!userid()) {
            $this->error('请先登录！');
        }
        ...
        //$userBank = M('UserBank')->where(array('id' => 6))->find();
		
		
		$userBank = M('UserBank')->where(array('userid' => userid(), 'status' => 1))->order('id desc')->limit(1)->find();
