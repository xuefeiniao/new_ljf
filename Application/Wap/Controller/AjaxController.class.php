	...
	public function imgUser(){
	    ...
	    if (!$info) {// 上传错误提示错误信息
	        ...
	        foreach ($info as $file) {
	            ...
	            //M('User')->where(array('id' => userid()))->setField('idcardimg'.$i, $path);
				M('User')->where(array('id' => userid()))->save(array('idcardimg'.$i=>$path,'senior_past'=>1));
	        }
	        //$this->ajaxReturn($path);
	        $this->ajaxReturn(array('code'=>'200','result'=>'操作成功'));
	    }
	    exit();
	}
...
// dump($data);
	...
	public function getEntrustAndUsercoin($market = NULL, $ajax = 'json')
	{
		...
		if ($userCoin) {
			...
			$data['usercoin']['xnbd'] = floatval($userCoin[$xnb . 'd']) + floatval($userCoin[$xnb . 'c']);
			$data['usercoin']['cny'] = floatval($userCoin[$rmb]);
			$data['usercoin']['cnyd'] = floatval($userCoin[$rmb . 'd']) + floatval($userCoin[$rmb . 'c']);
		}
		else {
			$data['usercoin'] = null;
		}

		$data['sumCoin'] = sumCoin(userid());//单位bdb
		...
		$data['buy_shouxufee'] = $buy_shouxufee;
		$data['sell_shouxufee'] = $sell_shouxufee;
		if ($ajax) {
	...
	public function is_bank(){
  	$uid = userid();
  	$info = M('user_bank')->where(array('userid'=>$uid))->getField('id');
  	if($info>0){
  ...
  public function is_ali(){
  	$uid = userid();
  	$info = M('user')->where(array('id'=>$uid))->getField('ali_img');
  	if($info != ''){
  		$this->ajaxReturn(array('code'=>200));
  	}else{
  		$this->ajaxReturn(array('code'=>404));
  ...
  public function is_weixin(){
  	$uid = userid();
  	$info = M('user')->where(array('id'=>$uid))->getField('weixin_img');
  	if($info != ''){
  		$this->ajaxReturn(array('code'=>200));
  	}else{
  		$this->ajaxReturn(array('code'=>404));
  ...
  public function is_renzheng(){
  	$uid = userid();
  	$info = M('user')->where(array('id'=>$uid))->field('senior_past')->find();
  ...
  public function is_shiming(){
  	$uid = userid();
  	$info = M('user')->where(array('id'=>$uid))->getField('idcardauth');
  		$this->ajaxReturn(array('code'=>200,'info'=>$info));
  	// if($info == 0){
  	// }else{
  		// $this->ajaxReturn(array('code'=>404));
