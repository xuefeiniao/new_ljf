				...
				if (!$Coin['zr_jz']) {
					...
					if (!$user_coin[$qbdz]) {
						...
						if ($Coin['type'] == 'qbb') {
							...
							if($coin=='eth'|| $coin=='eos'||$coin=='hpa')
							{
							$CoinClient = EthCommon($dj_address,$dj_port);
							$json =  $CoinClient->web3_clientVersion();
							//echo $json;die;
							if (!isset($json)) {
								$this->error('钱包链接失败！');
							}
							$qianbao_addr = $CoinClient->personal_newAccount(username());

							if (!is_array($qianbao_addr)) {
								$qianbao_ad = $CoinClient->personal_newAccount(username());

								if (!$qianbao_ad) {
									$this->error('生成钱包地址出错ECODE！');
								}
								else {
									$qianbao = $qianbao_ad;
								}
