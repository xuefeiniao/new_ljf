...
class FinanceController extends HomeController
{
    ...
    public function tx_history(){
        ...
        $list = $Moble->where($where)->order('id desc')->field('username,coinname,mum,fee,addtime')->select();
        foreach ($list as $k => $v) {
        	$list[$k]['addtime'] = date('Y-m-d H:i:s',$v['addtime']);
        	$list[$k]['coinname'] = strtoupper($v['coinname']);
          	$list[$k]['username'] = M('User_qianbao')->where(array('addr'=>strtoupper($v['username'])))->getField('name');
        }
        $this->ajaxReturn(array(
            'code'  =>  200,
            'result'   =>  '获取成功',
		...
		foreach ($CoinList as $k => $v) {
			if ($v['name'] == 'bdb') {
				...
				if(in_array($v['name'],C('coin_on'))){
					$coinList[$v['name']] = array('name' => strtoupper($v['name']), 'img' => $v['img'], 'title' => $v['title'] . '(' . strtoupper($v['name']) . ')', 'xnb' => round($UserCoin[$v['name']], 6) * 1, 'xnbd' => round($UserCoin[$v['name'] . 'd'], 6) * 1, 'xnbc' => round($UserCoin[$v['name'] . 'c'], 6) * 1,'xnbz' => round($UserCoin[$v['name']] + $UserCoin[$v['name'] . 'd']+$UserCoin[$v['name'] . 'c'], 6), 'jia' => $jia * 1, 'zhehe' => round(($UserCoin[$v['name']] + $UserCoin[$v['name'] . 'd']) * $jia, 2),'zc_fee'=>$v['zc_fee']);
				}	
				$cny['zj'] = round($cny['zj'] + (($UserCoin[$v['name']] + $UserCoin[$v['name'] . 'd']) * $jia), 2) * 1;
...
// 			$this->error('短信验证码错误！');
   ...
   public function mytxChexiao($id)
	...
	public function zr_coin(){
		...
		$where['status'] = 1;
		$coin = M("Coin")->where($where)->field('name,img')->select();
		foreach ($coin as $k => $v) {
			$coin[$k]['img'] = PC_URL."/Upload/coin/".$v['img'];
			$coin[$k]['name'] = strtoupper($v['name']); 
		}
		$this->ajaxReturn(array('code'=>200,'msg'=>$coin));
	}
	...
	public function myzr()
	{
	    ...
	    $userid = M('user')->where(array('login_token'=>$ltoken))->getField('id');
		$coin = strtolower(I("post.coin"));
		...
		$coninfo = M('Coin')->where(array('name'=>$coin))->getField('zr_min');
	...
	//app充值记录
	public function myzr_list(){
		$coin = I('request.coin');
	    $where['userid'] = userid();
	    $where['coinname'] = $coin;
	    $Moble = M('Myzr');
	    $list = $Moble->where($where)->order('id desc')->field('username,coinname,mum,addtime')->order('addtime desc')->select();
	    foreach ($list as $k => $v) {
	    	$list[$k]['addtime'] = date('Y-m-d H:i:s',$v['addtime']);
	    }
	   if($list){
	...
	public function turnout(){
		...
		$coin = strtolower($_POST['bz']);
		if($_POST['turnout'] == 1 && $_POST['turnin'] == 0){#从币币账户转向杠杆交易用户
		//dump(1111);
		...
		elseif($_POST['turnout'] == 0 && $_POST['turnin'] == 1){#从杠杆交易账户转向币币交易账户
			$coinnum = M('UserCoin')->where(array('userid'=>userid()))->getField('gg'.$coin);
			if($coinnum<$_POST['turnnum']){
				// $this->error('账户余额不足');
				$this->ajaxReturn(array('code'=>1007));
			}
			$bb_coin = M('UserCoin')->where(array('userid'=>userid()))->setInc($coin,$_POST['turnnum']);
			$gg_coin = M('UserCoin')->where(array('userid'=>userid()))->setDec('gg'.$coin,$_POST['turnnum']);
			if($bb_coin && $gg_coin){
				$transfer = M('Transfer')->add(array('userid'=>userid(),'bb_name'=>$coin,'gg_name'=>'gg'.$coin,'bbnum'=>'+'.$_POST['turnnum'],'ggnum'=>'-'.$_POST['turnnum'],'addtime'=>time(),'detail'=>'杠杆账户转向币币账户','type'=>2));
			}
		}else{
			// $this->error('操作失败！');
	...
	//转账账户余额
	public function turn_yue(){
		if (!userid()) {
			$this->error('请登录');
		}
		$coin = $_POST['bz'];
		$bb_yue = M('User_coin')->where(array('userid'=>userid()))->getField($coin);
		$gg_yue = M('User_coin')->where(array('userid'=>userid()))->getField('gg'.$coin);
		$this->ajaxReturn(array('code'=>200,'bb_yue'=>$bb_yue,'gg_yue'=>$gg_yue));
	}
	...
	//可提现数量
	public function tx_num(){
		$coin = I('post.coin');
		$num = M('User_coin')->where(array('userid'=>userid()))->getField($coin);
		$fee = M('Coin')->where(array('name'=>$coin,'status'=>1))->getField('zc_fee');
		$info = array('num'=>$num,'fee'=>$fee); 
		if($num){
	...
	//添加提现地址
	public function tx_addr()
	{
		$name = I('post.name');
		$addr = I('post.addr');
		$coin = strtoupper(I('post.coin'));
		$qianbao = M('user_qianbao');
		$result = $qianbao->where(array('userid'=>userid(),'coinname'=>$coin,'addr'=>$addr))->find();
		if($result)
		{
			$this->error('地址已存在');
		}
		$res = $qianbao->add(array('name'=>$name,'userid'=>userid(),'coinname'=>$coin,'addr'=>$addr,'addtime'=>time(),'status'=>1));
		if($res)
		{
			$this->success('添加成功');
		}
	...
	//提币地址列表
	public function tx_coin_list()
	{
		$coin = I('post.coin');
		$qianbao = M('user_qianbao')->where(array('userid'=>userid(),'coinname'=>$coin))->field('name,addr')->order('id desc')->select();
		foreach ($qianbao as $k => $v) {
			$qianbao[$k]['label'] = $v['name'];
			$qianbao[$k]['value'] = $v['addr'];
		}
		if($qianbao)
		{
			$this->success($qianbao);
		}
	}
	...
	public function upmyzc()
	{
		...
		$paypassword='';
		$moble_verify='';
		//$ltoken = 'ee1bf2e4f93e111343388bc27ea6955f';
		$userids = M('User')->where(array('login_token'=>$ltoken))->getField('id');
		//echo $userids;die;
		
		// $addr = M('user_qianbao')->where(array('name'=>$addr))->getField('addr');
		...
		//$userids = M('User')->where(array('login_token'=>$ltoken))->getField('id');
		$user_coin = M('UserCoin')->where(array('userid' => $userids))->find();
		// dump($user_coin[$coin]);die;
		//$coin = 'jtc';
     ...
     $coin = I('post.coin');
      if(($coin = 'amfc' || $coin == 'AMFC') && strlen($addr) >= 67)
    {
     
      $coin = 'amfc';
     // echo $coin.$userids;die;
            $user_res = M('User_coin')->where(array('userid'=>$userids))->setDec($coin,$num);
            if($user_res)
            {
              
             
                 $url = "http://suptoken.io/api/transaction/bourse";
                $mum = number_format($mum,8,'.','');
                $val = ['coin_en'=>'amfc','addr'=>$addr,'num'=>$mum];
               // var_dump($val);die;
                ksort($val);
                $_val = json_encode($val);
                $sign = sha1(md5($_val));

                $postFields = ['coin_en'=>'amfc','addr'=>$addr,'num'=>$mum,'sign'=>$sign];
               // var_dump($postFields);die;
                ...
                {
                  	$myzc_jl = M('Myzc')->add(array('userid'=>$userids,'username'=>$addr,'coinname'=>$coin,'txid'=>md5($addr . $user_coin[$coin . 'b'] . time()),'num'=>$num,'fee'=>$fee,'mum'=>$mum,'addtime'=>time(),'status'=>1));
                  if($myzc_jl){
                  	$this->success('转账成功');
                  }else
                {
     ...
     /* $coin = I('post.coin');
		...
		if($coin == 'amfc' || $coin == 'AMFC')
		{
          $coin == 'amfc';
          $from_addr = M('User_coin')->where(array('userid'=>$userids))->find();
			$url = "http://dapi.finsys.net/index.php/coin/jeff_zhuan_amfc";
			$mum = number_format($mum,8,'.','');
			$val = ['coin_en'=>'amfc','addr'=>$addr,'num'=>$mum];
			ksort($val);
			$_val = json_encode($val);
			$sign = sha1(md5($_val));
			$postFields = ['coin_en'=>'amfc','addr'=>$addr,'num'=>$mum,'sign'=>$sign,'from_addr'=>$from_addr['amfcb']];
			 //var_dump($postFields);die;
			$ch = curl_init ();
			curl_setopt( $ch, CURLOPT_URL, $url );
			curl_setopt( $ch, CURLOPT_RETURNTRANSFER, 1 );
        	...
        	if($res['status'] == 'ok')
        	{
        		$user_res = M('User_coin')->where(array('userid'=>$userids))->setDec('amfc',$num);
        		if($user_res)
        		{
        			$myzc_jl = M('Myzc')->add(array('userid'=>$userids,'username'=>$addr,'coinname'=>'amfc','txid'=>md5($addr . $user_coin['amfcb'] . time()),'num'=>$num,'fee'=>$fee,'mum'=>$mum,'addtime'=>time(),'status'=>1));
        			if($myzc_jl)
        			{
        				$this->success('转账成功');
        			}
      ...
      //$coin = I('post.coin');
    if(($coin = 'nwcc' || $coin == 'NWCC') && strlen($addr) >= 67)
    {
     
      $coin = 'nwcc';
     // echo $coin.$userids;die;
            $user_res = M('User_coin')->where(array('userid'=>userid()))->setDec($coin,$num);
            if($user_res)
            {
              $myzc_jl = M('Myzc')->add(array('userid'=>userid(),'username'=>$addr,'coinname'=>$coin,'txid'=>md5($addr . $user_coin[$coin . 'b'] . time()),'num'=>$num,'fee'=>$fee,'mum'=>$mum,'addtime'=>time(),'status'=>1));
              if($myzc_jl)
              {
                 $url = "http://jj.hunantianhe.com/api/bourse/extract";
                $mum = number_format($mum,8,'.','');
                $val = ['coin_en'=>'nwcc','addr'=>$addr,'num'=>$mum];
               
                ksort($val);
                $_val = json_encode($val);
                $sign = sha1(md5($_val));

                $postFields = ['coin_en'=>'nwcc','addr'=>$addr,'num'=>$mum,'sign'=>$sign];
               // var_dump($postFields);die;
                $ch = curl_init ();
                curl_setopt( $ch, CURLOPT_URL, $url );
                curl_setopt( $ch, CURLOPT_RETURNTRANSFER, 1 );
   ...
   // $coin = I('post.coin');
    if(($coin = 'jeff' || $coin == 'JEFF') && strlen($addr) >= 67)
    {
     ...
     // echo $coin.$userids;die;
            ...
            if($user_res)
            {
              ...
              if($myzc_jl)
              {
                 $url = "http://jj.hunantianhe.com/api/bourse/extract";
                $mum = number_format($mum,8,'.','');
                $val = ['coin_en'=>'jeff','addr'=>$addr,'num'=>$mum];
               
                ksort($val);
                $_val = json_encode($val);
                $sign = sha1(md5($_val));

                $postFields = ['coin_en'=>'jeff','addr'=>$addr,'num'=>$mum,'sign'=>$sign];
               // var_dump($postFields);die;
                $ch = curl_init ();
                curl_setopt( $ch, CURLOPT_URL, $url );
                curl_setopt( $ch, CURLOPT_RETURNTRANSFER, 1 );
...
$coin = I('post.coin');
		...
		if ($Coin['type'] == 'rgb') {
			...
			if (!$peer) {
				...
				$this->ajaxReturn(array('code'=>1047));
			}
		...
		if ($Coin['type'] == 'qbb') {
			...
			if ($mo->table('zhisucom_user_coin')->where(array($qbdz => $addr))->find()) {
				...
				$peer = M('UserCoin')->where(array($qbdz => $addr))->find();
			//error_log('qbdz='.$qbdz.'&&addr='.$addr.'&&peer='.$peer."\r\n",3,'./xx.txt');
				...
				$mo = M();
				$mo->execute('set autocommit=0');
				$mo->execute('lock tables  zhisucom_user_coin write  , zhisucom_myzc write  , zhisucom_myzr write , zhisucom_myzc_fee write,zhisucom_user write');
				$rs = array();
				$rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => userid()))->setDec($coin, $num);
				$rs[] = $mo->table('zhisucom_user_coin')->where(array('userid' => $peer['userid']))->setInc($coin, $mum);
				...
				$rs[] = $mo->table('zhisucom_myzc')->add(array('userid' => userid(), 'username' => $addr, 'coinname' => $coin, 'txid' => md5($addr . $user_coin[$coin . 'b'] . time()), 'num' => $num, 'fee' => $fee, 'mum' => $mum, 'addtime' => time(), 'status' => 1));
				$sql = $mo->table('zhisucom_myzc')->getLastSql();
				//error_log('sql='.$sql."\r\n",3,'./xx.txt');
				$rs[] = $mo->table('zhisucom_myzr')->add(array('userid' => $peer['userid'], 'username' => $user_coin[$coin . 'b'], 'coinname' => $coin, 'txid' => md5($user_coin[$coin . 'b'] . $addr . time()), 'num' => $num, 'fee' => $fee, 'mum' => $mum, 'addtime' => time(), 'status' => 1));
	 ...
	 die;*/
				...
				$mo->execute('lock tables  zhisucom_user_coin write, zhisucom_myzc write,zhisucom_myzr write, zhisucom_myzc_fee write');
				$rs = array();
				//$ress = array('userid' => userid(), 'username' => $addr, 'coinname' => $coin, 'num' => $num, 'fee' => $fee, 'mum' => $mum, 'addtime' => time(), 'status' => $auto_status);
				$rs[] = $r = $mo->table('zhisucom_user_coin')->where(array('userid' => $userids))->setDec($coin, $num);
				$auto_status=0;
				$insertdata=array('userid' => $userids, 'username' => $addr, 'coinname' => $coin, 'num' => $num, 'fee' => $fee, 'mum' => $mum, 'addtime' => time(), 'status' => $auto_status);
				$aid = $mo->table('zhisucom_myzc')->add($insertdata);
				//var_dump($insertdata);
				//$this->ajaxReturn($aid);die;
				if ($fee && $auto_status) {
					//$this->error(111);die;
					$rs[] = $mo->table('zhisucom_myzc_fee')->add(array('userid' => $fee_user['userid'], 'username' => $Coin['zc_user'], 'coinname' => $coin, 'num' => $num, 'fee' => $fee, 'mum' => $mum, 'type' => 2, 'addtime' => time(), 'status' => 1));
				...
				if (check_arr($rs)) {
					if ($auto_status) {
						...
						if (!$flag) {
							...
							//$this->ajaxReturn(array('code'=>1049));
						}
						else {
							 $this->success('转出成功!');
							//$this->ajaxReturn(array('code'=>1012));
  ...
  public function mywc()
  {
	...
	public function wallet_zr()
  	{
  	$coin = I('post.coin_en');
    $addr = I('post.addr');
    $sign = I('post.sign');
    $num = I('post.num');
   // $wallet_sign = md5(md5('wallet_sign'.$num.$addr).$coin);
      
      $mum = number_format($num,8,'.','');
      $val = ['coin_en'=>$coin,'addr'=>$addr,'num'=>$mum];

      ksort($val);
      $_val = json_encode($val);
      $wallet_sign = sha1(md5($_val));
    ...
    $user_addr = M('User_coin')->where(array($coin.'b'=>$addr))->find();
    if(!$user_addr){
    	//$this->error('地址不存在');
      $this->ajaxReturn(array(
            'code'  =>  0,
    ...
    $usercoin = M('User_coin')->where(array('id'=>$user_addr['id']))->setInc($coin,$num);
    $myzr = M('Myzr')->add(array('userid'=>$user_addr['userid'],'username'=>$addr,'coinname'=>$coin,'txid'=>md5($addr.$coin.$num.$sign),'num'=>$num,'mum'=>$num,'fee'=>0,'addtime'=>time(),'status'=>1));
  ...
  public function jeff_price()
  {
  	$price = round(M('Market')->where(array('name'=>'usdt_jeff'))->getField('new_price'),4);
